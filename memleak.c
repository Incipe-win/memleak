...
static int libbpf_print_fn(enum libbpf_print_level level, const char *format, va_list args)
{
    ...
    return 0; /* suppress debug/info output */
}
...
static void show_stack_trace(__u64 *stack, int stack_sz, pid_t pid)
{
    ...
    for (i = 0; i < stack_sz; i++) {
        if (!result || result->cnt <= i || !result->syms[i].name) {
            printf(" %2d [<%016llx>]\n", i, stack[i]);
            continue;
        }
        ...
        if (sym->code_info.dir && sym->code_info.dir[0] != '\0' &&
            sym->code_info.file && sym->code_info.file[0] != '\0') {
            printf(" %2d [<%016llx>] %s+0x%lx %s/%s:%u\n", i, stack[i],
                   sym->name, sym->offset, sym->code_info.dir,
                   sym->code_info.file, sym->code_info.line);
        } else if (sym->code_info.file && sym->code_info.file[0] != '\0') {
            printf(" %2d [<%016llx>] %s+0x%lx %s:%u\n", i, stack[i],
                   sym->name, sym->offset, sym->code_info.file,
                   sym->code_info.line);
        } else {
        ...
        for (j = 0; j < sym->inlined_cnt; j++) {
            ...
            if (inlined->code_info.dir && inlined->code_info.dir[0] != '\0' &&
                inlined->code_info.file && inlined->code_info.file[0] != '\0') {
                printf("        %s+0x%lx %s/%s:%u\n", inlined->name,
                       (unsigned long)0, inlined->code_info.dir,
                       inlined->code_info.file, inlined->code_info.line);
            } else if (inlined->code_info.file && inlined->code_info.file[0] != '\0') {
                printf("        %s+0x%lx %s:%u\n", inlined->name,
                       (unsigned long)0, inlined->code_info.file,
                       inlined->code_info.line);
            } else {
                printf("        %s+0x%lx\n", inlined->name, (unsigned long)0);
            }
        }
    }
